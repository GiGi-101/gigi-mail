    ...
    def run(self):
        ...
        try:
            with imaplib.IMAP4_SSL("imap.gmail.com") as mail_server:
                ...
                if " " in actual_folder and not (
                    actual_folder.startswith('"') and actual_folder.endswith('"')
                ):
                ...
                loaded_msg = email.message_from_bytes(
            ...
            logging.error(
                f"Fehler im MailContentWorker beim Laden von Mail {self.mail_id}: {e}",
                exc_info=True,
            )
        ...
        try:
            with imaplib.IMAP4_SSL("imap.gmail.com") as mail_server:
                ...
                if self.top_mail_id is None:
                    logging.info(
                        f"Initialer Ladevorgang für Ordner '{actual_folder}' gestartet."
                    )
                    ...
                    status, fetch_data = mail_server.fetch(
                        id_string, "(RFC822.HEADER FLAGS)"
                    )
                    if status != "OK":
                    ...
                    for i in range(num_items - 2, -1, -2):
                        ...
                        if isinstance(tuple_item, tuple) and isinstance(
                            flags_item, bytes
                        ):
                            loaded_msg = email.message_from_bytes(
                                tuple_item[1], policy=email.policy.default
                            )
                    ...
                    logging.info(
                        f"Neue E-Mails in '{actual_folder}' gefunden! Neueste ID: {newest_mail_id}"
                    )
                    # Filter IDs to fetch headers only for newly arrived emails
                    new_ids = [
                        mid for mid in mail_ids if int(mid) > int(self.top_mail_id)
                    ]
            ...
            logging.error(
                f"Fehler im MailWorker bei Ordner '{self.folder}': {e}", exc_info=True
            )
        finally:
            self.finished.emit()
